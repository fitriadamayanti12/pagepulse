...
export default function DiscussionForm({ formData, submitting, onFormChange, onSubmit, onClose }: DiscussionFormProps) {
  return (
    <div className="bg-white/60 backdrop-blur-xl rounded-3xl border-2 border-amber-100/40 shadow-2xl mb-6 overflow-hidden animate-in fade-in slide-in-from-top-4">
      <div className="bg-gradient-to-r from-amber-50 to-orange-50 px-6 py-4 border-b border-amber-100/40">
        <div className="flex justify-between items-center">
          <h2 className="text-xl font-extrabold text-[#3d3530]">Create New Topic</h2>
        ...
        <form onSubmit={onSubmit} className="space-y-5">
          ...
          <div className="flex gap-3 pt-2">
            <Button type="submit" disabled={submitting}
              ...
              {submitting ? <div className="flex items-center gap-2"><div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" />Creating...</div> : 'Create Topic'}
            </Button>

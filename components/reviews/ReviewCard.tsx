  ...
  return (
    <div className="bg-white/50 backdrop-blur-xl rounded-2xl border-2 border-amber-100/40 p-5 sm:p-6 shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300">
      <div className="flex items-start justify-between gap-4">
        ...
        {isOwner && (
          <div className="flex gap-1 flex-shrink-0">
            <button onClick={() => onEdit(review)} className="p-2.5 text-[#9b8d80] hover:text-amber-600 hover:bg-amber-50/60 rounded-xl transition-all" title="Edit">
              <Edit2 className="w-5 h-5" />
            </button>
            <button onClick={() => onDelete(review.id)} className="p-2.5 text-[#9b8d80] hover:text-rose-500 hover:bg-rose-50/60 rounded-xl transition-all" title="Delete">
      ...
      <div className="mt-5 pt-4 border-t border-amber-100/40">
        <button onClick={() => onLike(review)} className="flex items-center gap-2 text-[#9b8d80] hover:text-rose-500 transition-colors font-bold text-sm">
